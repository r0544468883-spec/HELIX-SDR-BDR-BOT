...
h1{font-size:20px;margin:0 0 8px}p{color:#9fb0c3;margin:0;line-height:1.6}
...
export async function GET(request: NextRequest, ctx: { params: Promise<{ token: string }> }) {
  ...
  if (action !== 'confirm' && action !== 'cancel') return page('קישור לא תקין', 'הפעולה המבוקשת אינה מזוהה.');
